                    ...
                    <x-notification-dropdown align="right" width="96">
                        <x-slot name="trigger">
                            <button class="relative p-2 text-gray-400 hover:text-gray-500 focus:outline-none">
                                ...
                                <span x-data="{ count: 0 }"
                                      x-init="async () => { const r = await fetch('{{ route('notifications.unread-count') }}'); const d = await r.json(); count = d.count }"
                                      x-show="count > 0"
                                      class="absolute top-1 right-1 inline-flex items-center justify-center h-4 w-4 text-xs font-bold text-white bg-red-500 rounded-full"
                                      x-text="count">
                                </span>
                            </button>
                        </x-slot>
